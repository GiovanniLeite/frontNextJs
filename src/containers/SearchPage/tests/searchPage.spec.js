...
describe('Containers <SearchPage />', () => {
  it('should render <SearchPage />', () => {
    const { getByText, getAllByTestId } = customRender(
      <SearchPage posts={mockedPosts} search="Acade" />,
    );

    expect(getByText(/Você procurou por: Acade/)).toBeInTheDocument();
    expect(getByText('Academia top Mais')).toBeInTheDocument();
    expect(getAllByTestId('coverPost')[0]).toBeInTheDocument();
    expect(getByText('MyFit Academia')).toBeInTheDocument();
