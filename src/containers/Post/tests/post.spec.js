...
describe('Containers <Post/>', () => {
  it('should render <Post/>', () => {
    const { getByText, getByTitle } = customRender(<Post post={mockedPost} />);

    expect(getByText('Academia top Mais')).toBeInTheDocument();
    expect(getByTitle('_2.png')).toBeInTheDocument();
    expect(getByText(/Sic dum rursusque laborent securi/)).toBeInTheDocument();
  });
});
